...
function deriveSymbol(name: string): string {
  ...
  const symbol = (initials.length >= 2 ? initials : (words[0] ?? "")).slice(
    0,
    8,
  );
  return symbol || "ITEMS";
...
export function CreateForm() {
      ...
      if (missing.length > 0) {
        setCustomChainError(
          `No token found on ${missing
            ...
            .join(
              ", ",
            )} — it must exist at this address on every selected chain.`,
        );
        return;
      }
  ...
  const toStageRules = (
    ...
    return {
      ...
      cashOutTaxRate:
        stage.cashOuts && !routedAll ? stageCashOutTax(stage) : null,
        ...
        if (hash) {
          ...
          hash = await submitReviewedContractWrite({
            ...
            switchChain: (reviewedChainId) =>
              switchChainAsync({
                chainId: reviewedChainId as SupportedChainId,
              }),
      ...
      <div className="relative mx-auto max-w-2xl px-4 py-14 sm:px-6 sm:py-20">
        <div className="card p-8 text-center sm:p-10">
          ...
          <p className="mt-2 text-sm text-smoke-700">
            Your project page is being indexed — it usually takes about a minute
  ...
  return (
    ...
    >
      ...
      <section
        className={
          !isSimpleProject && step === 2 ? "card mt-6 p-6 sm:p-7" : "hidden"
        }
      >
        <div className="flex items-center gap-3">
          <StepBadge n={3} />
        ...
        <div className="mt-5">
          ...
          {flavor !== "revnet" && deadlineApplies ? (
            <div className="mt-5 border-t border-smoke-200 pt-4">
              ...
              <p className="mt-1 text-xs leading-relaxed text-smoke-700">
                The condition a queued rule change must satisfy before it takes
                effect, giving supporters time to review changes.
              </p>
              <select
              ...
              {approvalDeadline === "custom" ? (
                <div className="mt-3">
                  <p className="mb-2 text-xs leading-relaxed text-smoke-700">
                    ...
                    queued changes. It must be deployed on every selected chain.
                  </p>
                  <AddressField
                  ...
                  {selected.length > 1 ? (
                    <div className="mt-2 space-y-2">
                      <p className="text-[11px] text-smoke-500">
                        Different address on some chains? Set per chain (empty =
                        the address above):
                      </p>
                      {selected.map((chainId) => (
                        <div key={chainId} className="flex items-start gap-2">
      ...
      >
        ...
        <div className="mt-4">
          ...
          <p className="mt-1 text-xs leading-relaxed text-smoke-700">
            The currency your items are priced in — buyers can still pay with
            anything.
          </p>
          <div
            role="radiogroup"
        ...
        {customOn && effectiveStoreCurrency !== "token" ? (
          <p className="mt-2 text-xs leading-relaxed text-peel-600">
            Pricing in ETH or USD with a custom accounting token needs a price
            feed added after launch.
          </p>
        ) : null}
      ...
      >
        ...
        <div className="mt-5">
          <CheckRow
            ...
            blurb="Juicebox is experimental onchain software. Your project is controlled by its rules and project owner — not by Juicebox — and launches are irreversible."
          />
        </div>
