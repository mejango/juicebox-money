...
function contractName(chainId: JBChainId, address: Address): string | null {
  ...
  for (const [name, deployments] of Object.entries(contracts)) {
    if (deployments?.[chainId]?.toLowerCase() === address.toLowerCase()) return name
  }
  return null
}
...
function executionPlan(
  ...
  for (const transaction of byNonce.get(currentNonce) ?? []) direct.add(transaction)
...
export function SafeQueueCard({
  ...
  const confirmExecuteAll = async () => {
    ...
    setBusy('execute-all')
    setError(null)
    setNotice('Confirm one Relayr payment to execute every reviewed transaction.')
    try {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-smoke-700">
            {authorityLabel}-only actions are proposed per chain. Safe signers can
            inspect, co-sign, and execute them here.
          </p>
        </div>
        {ready.length >= 2 ? (
          <button
      ...
      {query.isLoading ? (
        ...
        <div className="mt-4 space-y-5">
          {(query.data ?? []).map(chain => {
            ...
            return (
              <div key={chain.chainId} className="rounded-xl border border-smoke-200">
                ...
                {!chain.info ? (
                  ...
                  <p className="px-4 py-4 text-sm leading-relaxed text-smoke-700">
                    ...
                    can reopen the same action to approve its exact hash onchain;
                    the app executes it automatically when the Safe’s threshold
                    is met.
                  </p>
                ) : chain.error ? (
                  <p className="px-4 py-4 text-sm text-red-700">{chain.error}</p>
                  ...
                  <ul className="divide-y divide-smoke-100">
                    {chain.transactions.map(tx => {
                      ...
                      const href = hash
                        ...
                        : queueUrl
                      return (
                        <li key={`${tx.nonce}:${hash ?? tx.data}`} className="px-4 py-3.5">
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            <details className="min-w-0 flex-1">
                              <summary className="cursor-pointer list-none">
                                <p className="text-sm font-medium text-ink">
                                  #{tx.nonce} · {transactionLabel(chain.chainId, tx)}
                                </p>
                                <p className="mt-1 text-xs text-smoke-500">
                                  {count}/{required} signatures
                                  {readyToExecute ? ' · ready' : ''}
                                  {alternative ? ' · same-nonce alternative' : ''}
                                  {Number(tx.operation) === 1 ? ' · DELEGATECALL' : ''}
                                ...
                                <p className="mt-1 text-xs text-smoke-500">
                                  ...
                                  {tx.confirmations?.length
                                    ? tx.confirmations
                                        .map(item => truncateAddress(item.owner))
                                        .join(', ')
                                    : 'none'}
                                </p>
                              ...
                              <div className="mt-3 rounded-lg bg-smoke-50 p-3 font-mono text-[11px] leading-relaxed text-smoke-700">
                                ...
                                <p>Operation: {Number(tx.operation) === 1 ? 'DELEGATECALL' : 'CALL'}</p>
                                <p>Value: {tx.value ?? 0} wei</p>
                                <p className="mt-1 break-all">Data: {tx.data ?? '0x'}</p>
                              </div>
                            </details>
                            <div className="flex shrink-0 flex-wrap items-center gap-2">
                              ...
                              {readyToExecute ? (
                                ...
                                >
                                  {busy === `execute:${chain.chainId}:${tx.nonce}`
                                    ? 'Executing…'
                                    : 'Execute'}
                                </button>
                              ) : null}
