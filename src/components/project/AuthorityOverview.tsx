    ...
    deployments.map(async deployment => {
      ...
      if (authority) {
        ...
        if (safe) accountType = 'Safe Multisig'
          ...
          const code = await client.getBytecode({ address: authority }).catch(() => null)
...
function aggregateGrants(
  ...
  for (const holder of holders) {
    ...
    current.union = [...new Set([...current.union, ...holder.permissions])].sort(
      (a, b) => a - b,
    )
...
export function AuthorityOverview({
  ...
  const differs =
    known.length > 1 &&
    known.some(
      row => row.authority!.toLowerCase() !== known[0].authority!.toLowerCase(),
    )
  const safeGroups = useMemo(() => {
    const byAddress = new Map<string, { safe: Address; rows: AuthorityRow[] }>()
    for (const row of rows) {
      ...
      <section className="card p-5">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-smoke-700">
            {isRevnet
              ? 'Revnets have no owner. The operator holds only the permissions granted at launch, and can pass that role on.'
              : 'The project NFT is ownership. Its owner controls owner-only actions, either directly or through a Safe.'}
          </p>
        </div>
        {authorityQuery.isLoading ? (
          ...
          <div className="mt-4 space-y-4">
            ...
            {groups.map(group => {
              ...
              return (
                <div key={group.key} className="rounded-xl border border-smoke-200 p-4">
                  ...
                  group.accountType === 'Contract' ? (
                    <p className="mt-3 text-xs leading-relaxed text-smoke-500">
                      This authority is a non-Safe contract. Submit owner actions
                      through that contract’s governance or wallet interface.
                    </p>
                  ) : null}
                </div>
              )
...
function DeploySafeButtons({
  ...
  const deploy = async (row: AuthorityRow) => {
    ...
    try {
      ...
      if (!creation) throw new Error('Could not read the Safe’s creation config.')
      ...
      setMessage(error instanceof Error ? error.message : 'Could not deploy the Safe.')
    } finally {
      setBusy(null)
    }
  }
  ...
  return (
    <div className="mt-3">
      <p className="text-xs leading-relaxed text-smoke-700">
        This is the same Safe address used on another chain, but its proxy is not
        deployed here yet. Replay its canonical creation to activate it.
      </p>
      ...
      {message ? <p className="mt-2 text-xs text-smoke-700">{message}</p> : null}
    </div>
  )
}
...
function TransferAuthorityFlow({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const queued = result.safeResults.filter(item => item.status === 'queued').length
      const waiting = result.safeResults.filter(item => item.status === 'waiting').length
      setStatus(
        queued || waiting
          ? `Your Safe action is recorded${queued ? `; ${queued} queued` : ''}${waiting ? `; ${waiting} awaiting more onchain approvals` : ''}.`
          : `${isRevnet ? 'Operator' : 'Ownership'} transferred on ${rows.length} chain${rows.length === 1 ? '' : 's'}.`,
      )
      onDone()
    } catch (submitError) {
      setError(
        submitError instanceof Error ? submitError.message : 'Could not transfer.',
      )
    } finally {
      setBusy(false)
    }
    ...
    <div className="mt-4 rounded-xl border border-smoke-200 p-4">
      ...
      <div className="mt-3">
        <AddressField
          ...
          placeholder={
            isRevnet
              ? '0x… new operator (zero address relinquishes)'
              : '0x… new owner'
          }
          ariaLabel={isRevnet ? 'New operator' : 'New owner'}
        />
      </div>
        ...
        <span className="text-xs leading-relaxed text-red-700">
          {isRevnet
            ? destination.trim().toLowerCase() === zeroAddress
              ? 'I understand that relinquishing the operator is permanent.'
              : 'I verified the new operator. They receive every power attached to this role.'
            : 'I verified the new owner. Transferring the project NFT hands over every owner-only power.'}
        </span>
      </label>
      <button
        type="button"
...
function PermissionsAcrossChains({
  ...
  return (
    <section className="card p-5">
      ...
      <p className="mt-2 text-sm leading-relaxed text-smoke-700">
        {isRevnet
          ? 'Every power the revnet’s operator role currently holds, including any NFT powers granted at launch.'
          : 'Operators authorized to act for the owner. Each row shows exactly what is granted and on which chains.'}
      </p>
            ...
            <div key={grant.operator} className="py-4 first:pt-0 last:pb-0">
              ...
              <div className="mt-3 space-y-2">
                {grant.union.map(id => {
                  ...
                  return (
                    <div key={id} className="grid gap-1 sm:grid-cols-[13rem_1fr_auto] sm:gap-3">
                      ...
                      <span className="flex items-center gap-1" title="Granted on">
                        {onChains.map(chainId => (
                          <ChainIcon key={chainId} chainId={chainId} size={16} />
                        ))}
                      </span>
                    </div>
                  )
...
function PermissionEditor({
  ...
  const [selectedChains, setSelectedChains] = useState<Set<number>>(
    () =>
      new Set(
        grant?.rows.map(row => row.chainId) ?? deployments.map(row => row.chainId),
      ),
  )
  const [ack, setAck] = useState(false)
  const [busy, setBusy] = useState(false)
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const deployment of chosen) {
        ...
        if (!authority) throw new Error(`Owner is unknown on chain ${deployment.chainId}.`)
        ...
        const unknownIds = decodePermissionBitmap(bitmap).filter(id => id > 39)
