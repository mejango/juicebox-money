...
const STORE_KEY = 'jbm:query-cache:v1'
...
/**
 * Drop-in `meta` for a read whose value is worth showing from the last session
 * while it revalidates. Pair it with `Revalidating` so the stale value reads as
