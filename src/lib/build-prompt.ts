...
Design the smallest safe architecture that gives my users a native product experience while Juicebox handles the money layer. Decide whether I need a flexible Juicebox project, a revnet with a precommitted economic schedule, or both. Map every user action to exact V6 reads and transactions, including payments, token issuance, cash outs, payouts, shops, hooks, permissions, and multichain settlement where relevant.

For each transaction, identify the contract, function, arguments, units, permissions, fees, approvals, slippage or minimum-output protection, and the state that must be re-read immediately before signing. Use current V6 SDK builders where available, verify them against the deployed contracts, and round-trip pure transaction builders through the ABI. Never ask me to connect a wallet or sign until you have shown me the decoded transaction and explained its effect.
